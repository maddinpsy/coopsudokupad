import { PortableEvents } from './portableevents.js'
import { throttleFunc } from './utilities.js'
...
const Framework = (() => {
	...
	F.showAlert = (message, label = 'OK') => F.showDialog({
		parts: [
	...
	F.renderAd = async (slot, ad) => {
		try {
			...
			if (!showAd && (el = document.querySelector('.supportlinks'))) {
				Object.assign(el, { style: '', innerHTML: html });
				showAd = true;
			}
