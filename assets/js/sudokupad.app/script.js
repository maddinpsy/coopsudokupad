...
import { toRC, stepPoints, isSameRC, roundCenter, getCellsAlongPoints, cssColorHasAlpha, calcDistance, triangularNumber } from './utilities.js'
...
const SvgRenderer = (() => {
	...
	P.createArrowHead = function (opts = {}) {
		...
		marker.appendChild(this.renderPart({
			type: 'path', attr: Object.assign({ fill: color }, style.attr,
	...
	P.getChildElem = function (className, opts) {
		...
		if (this.childElems[className] === undefined) {
			...
			if (['cell-color', 'cell-highlight', 'cell-grid', 'cell-error'].includes(className)) {
				elem = svg.renderRect(Object.assign({
					target, className,
					'vector-effect': 'non-scaling-stroke',
					'vector-effect': 'non-scaling-size',
				...
				elem = svg.renderText(Object.assign({
					target, className,
					center: [this.row + 0.5, this.col + 0.5],
					width: 1, height: 1,
					//text: this.value,
...
const Timer = (() => {
	...
	P.createUI = function (parent) {
		...
		createElem({
			class: 'timer-control', parent, children: [
...
const Checker = (() => {
	...
	Checker.checkCellConflicts = function (errors, cell, seenCells) {
		const cellVal = cell.getVal();
		if (cell.hideclue || cellVal === undefined) return;
		for (let i = 0, len = seenCells.length; i < len; i++) {
			const seencell = seenCells[i];
			if (cell === seencell) continue;
			if (cellVal !== (seencell.given || seencell.value)) continue;
			errors.push({
	...
	Checker.checkCages = function (puzzle, cages = [], errors = []) {
		...
		cages.forEach(cage => {
			...
			let cellValues = cells.map(cell => cell.given || cell.value).filter(val => val !== undefined);
	...
	Checker.check = function (puzzle, features = []) {
		...
		*/
		...
		for (let i = 0, len = cells.length; i < len; i++) {
			...
			const seenCells = seenCageCells(cell, puzzleInfo);
...
const Puzzle = (() => {
	...
	// Loading
	P.getMinMaxRC = function () {
		const { grid, grid: { rows, cols }, currentPuzzle = {} } = this;
		const { cages = [], solution } = currentPuzzle;
	...
	C.SmartSelectToolOrder = {
		normal: ['normal', 'colour', 'centre', 'corner', 'thermocosmetic', 'arrowsumcosmetic', 'cellcosmetic'],
		centre: ['normal', 'centre', 'corner', 'colour', 'thermocosmetic', 'arrowsumcosmetic', 'cellcosmetic'],
		corner: ['normal', 'corner', 'centre', 'colour', 'thermocosmetic', 'arrowsumcosmetic', 'cellcosmetic'],
		colour: ['colour', 'normal', 'centre', 'corner', 'thermocosmetic', 'arrowsumcosmetic', 'cellcosmetic'],
	};
	...
	C.makeSelector = (puzzle, cell, tool) => {
		...
		switch (tool) {
			case 'normal':
				selVal = (!cell.hideclue && cell.propGet('given')) || cell.propGet(tool) || undefined;
				return (selVal !== undefined) && (c => c.getVal() === selVal);
			case 'centre':
				selVal = [...cell.propGet(tool)];
				return (selVal.length > 0) && (c => c.propVisible(tool) && selVal.find(val => !c.propContains(tool, val)) === undefined);
			case 'corner':
				selVal = [...cell.propGet(tool)];
				return (selVal.length > 0) && (c => c.propVisible(tool) && selVal.find(val => c.propContains(tool, val)) !== undefined);
			case 'colour':
	...
	const cageCosmetic = ({ cages = [] }) => {
		...
		for (const { value, cells = [] } of cages) {
			for (const cell of cells) {
				res.push({ type: 'cellcosmetic', rc: toRC(cell), cosmetic: `cage-${cells.length}-${value !== undefined ? value : ''}` });
			}
		}
		return res;
	};
	const cosmetic = puzzle => [cellCosmetic, thermoCosmetic, arrowsumCosmetic, cageCosmetic].flatMap(fn => fn(puzzle));
...
function ensureLocalStorageQuota() {
	...
	Framework.showDialog({
		parts: [
			...
			{ tag: 'options', options: [`Delete Old Progress`, 'Cancel'] },
		],
